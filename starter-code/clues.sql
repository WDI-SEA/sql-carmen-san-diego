...
-- so find the least populated country in Southern Europe, and we'll start looking for her there.
    ...
    SELECT name, continent, region, population FROM countries WHERE region='Southern Europe' ORDER BY population DESC;
    SELECT name='Holy See (Vatican Cities State)' FROM countries
...
--  nearby country speaks nothing but that language.
    SELECT language, countrycode FROM countrylanguages WHERE language='Italian' AND percentage='100';;
...
-- Clue #4: We're booking the first flight out – maybe we've actually got a chance to catch her this time.
 ...
 --  be flying to.
    SELECT name FROM cities WHERE countrycode='SMR' AND NOT name = (SELECT name FROM countries WHERE code='SMR');
