...
-- will give me a country code = VAT 
...
SELECT countries.region , cities.name FROM cities 
JOIN  countries  ON countries.code = cities.countrycode 
WHERE countries.region = 'South America'
AND
cities.name LIKE 'Serra%'

-- returns a city called "Serra"
...
SELECT * FROM cities
WHERE cities.id = (SELECT countries.capital FROM cities 
JOIN countries ON countries.code = cities.countrycode
WHERE cities.name = 'Serra')

-- the name is Bras�lia
