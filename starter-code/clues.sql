...
-- NOTES From class review to find country name from code:
SELECT * FROM countries WHERE code = 'SMR';

-- NOTES From class - nested queries:
SELECT * FROM countries WHERE code = (
    -- nest sql statement will run FIRST
    SELECT countrycode FROM countrylanguages WHERE language = 'Italian' AND percentage = 100 --;
    -- ***** NO SEMI-COLON AFTER A NESTED STATEMENT *****
);
...
-- Clue #4: We're booking the first flight out – maybe we've actually got a chance to catch her this time.
 ...
 -- ***** NOTES From class - use 'NOT' to exclude a result (like != in javascript) *****
...
SELECT * FROM cities WHERE name LIKE 'Serr%'; --(note the S MUST be capital)

-- NOTES from class - use 'LOWER()' function to search for letter as lower case
...
-- NOTES from class - use nested statements to get result in one query 
...
-- NOTES from class
-- ***** USE '\i' + the name of the sql file (i.e. 'starter-code/clues.sql') to run queries in terminal 
